...
/**
 * \file    test/phpunit/WebObserverFunctionalTest.php
 * \ingroup webobserver
...
/**
 * Class WebObserverFunctionalTest
 *
 ...
 *
 * @package Testwebobserver
 */
class WebObserverFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modWebObserver')->click();
		} else {
			// Disable the module
			$this->byHref('modWebObserver')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/webobserver/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('webobserver/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/webobserver/admin/about.php');
		$this->authenticate();
		return $this->assertContains('webobserver/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('webobserverwidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modWebObserver_WebObserverTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modWebObserver_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
