...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/webobserver/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
			$filefound = 1;
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'WEBOBSERVER_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		$tmpobjectkey = GETPOST('object');
		if (!empty($tmpobjectkey)) {
			$constforval = 'WEBOBSERVER_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
		...
		foreach ($arrayofparameters as $constname => $val) {
			if ($val['enabled']==1) {
				...
				if ($val['type'] == 'textarea') {
					...
					require_once DOL_DOCUMENT_ROOT.'/core/class/html.formATM Consulting - GPAO.class.php';
					$formATM Consulting - GPAO = new FormOther($db);

					$tmp = explode(':', $val['type']);
					print img_picto('', 'category', 'class="pictofixedwidth"');
					print $formATM Consulting - GPAO->select_categories($tmp[1],  $conf->global->{$constname}, $constname, 0, $langs->trans('CustomersProspectsCategoriesShort'));
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'WEBOBSERVER_'.strtoupper($myTmpObjectKey).'_ADDON';
